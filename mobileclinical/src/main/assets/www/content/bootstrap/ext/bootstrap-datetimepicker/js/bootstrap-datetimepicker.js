(function($) {
	...
	var $proxy = function(fn,scope){
		...
		if(arguments.length > 2){
			for(var i = 2; i<arguments.length; i++) fn.pre_args[i-2] = arguments[i];	
		}
		return function(){
			var args = [];
			...
			fn.apply(scope,fn.pre_args.concat(args));
		}
	}
			...
			if (this.pickDate && this.pickTime) {
				...
				function(e) {
					...
					if (expanded && expanded.length) {
						...
						self.$element.find(".input-group-addon").toggleClass(self.timeIcon + " " + self.dateIcon)
					}
				})
			}
			if (this.isInput) {
